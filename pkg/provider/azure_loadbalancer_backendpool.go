...
type BackendPool interface {
	...
	ReconcileBackendPools(clusterName string, service *v1.Service, lb *network.LoadBalancer) (bool, bool, bool, *network.LoadBalancer, error)
...
func (bc *backendPoolTypeNodeIPConfig) ReconcileBackendPools(clusterName string, service *v1.Service, lb *network.LoadBalancer) (bool, bool, bool, *network.LoadBalancer, error) {
			...
			if bp.BackendAddressPoolPropertiesFormat != nil &&
				...
				if removeNodeIPAddressesFromBackendPool(bp, []string{}, true) {
					...
					if err := bc.CreateOrUpdateLBBackendPool(lbName, bp); err != nil {
						...
						return false, false, false, nil, fmt.Errorf("bc.ReconcileBackendPools for service (%s): failed to cleanup IP based backend pool %s: %w", serviceName, lbBackendPoolNames[isIPv6], err)
					}
	...
	if backendPoolsUpdated {
		...
		if err != nil {
			return false, false, false, nil, fmt.Errorf("bc.ReconcileBackendPools for service (%s): failed to get loadbalancer %s: %w", serviceName, lbName, err)
		}
	}
	...
	return isBackendPoolPreConfigured, backendPoolsCreated, backendPoolsUpdated, lb, err
}
...
func (bi *backendPoolTypeNodeIP) ReconcileBackendPools(clusterName string, service *v1.Service, lb *network.LoadBalancer) (bool, bool, bool, *network.LoadBalancer, error) {
	var newBackendPools []network.BackendAddressPool
	if lb.BackendAddressPools != nil {
		newBackendPools = *lb.BackendAddressPools
	}
		...
		for _, i := range bpIdxes {
			...
			if updated {
				...
				if err := bi.CreateOrUpdateLBBackendPool(lbName, bp); err != nil {
					return false, false, false, nil, fmt.Errorf("bi.ReconcileBackendPools for service (%s): lb backendpool - failed to update backend pool %s for load balancer %s: %w", serviceName, pointer.StringDeref(bp.Name, ""), lbName, err)
				}
	...
	if shouldRefreshLB {
		...
		if err != nil {
			return false, false, false, nil, fmt.Errorf("bi.ReconcileBackendPools for service (%s): failed to get loadbalancer %s: %w", serviceName, lbName, err)
		}
	}
	...
	isOperationSucceeded = true
	return isBackendPoolPreConfigured, backendPoolsUpdated, shouldRefreshLB, lb, nil
}
